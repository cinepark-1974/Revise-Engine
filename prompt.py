...
#
# v3.4.0 (2026-06-23) ★ A35 신규 룰 — 씬 헤딩 시간대 5단계 한글 표기 강제.
# - 「상속」 R2에서 시간대 표기 혼용 결함 발견 (DAY/NIGHT 영문 + 낮/밤/점심/아침 혼재).
# - 시간대를 새벽/오전/오후/저녁/밤 5개로만 강제. 그 외 표기 전부 금지.
# - 매핑: 점심·정오→오후, 해질녘·황혼→저녁, 아침→오전, DAY/낮→맥락 판단(기본 오후).
# - 집필 단계 강제(A35 룰) + 출력 단계 자동 후처리(auto_fix_a35_scene_time, main.py) 이중 방어.
# =================================================================
...
v3.3.5 신규 → v3.3.7 강화. 「오랜만에」 R2 작업에서 발견된 7가지 LLM 결함 방어.

LLM이 처방을 받을 때 자주 발생하는 치명적 실수를 방지하는 룰이다.
v3.3.7에서 결함 패턴 4·5·6·7이 추가되었다 — 짧은 응답 라인의 무변주 반복,
씬 간 미러링 다이얼로그 패턴 동일 복사, 모티프(소품·객체) 반복, 핵심 답변
라인의 글자 단위 복제. 자동 후처리 함수 auto_fix_a33_violations이 이 7가지를
모두 잡아낸다.

┌─────────────────────────────────────────────────────────────────┐
│ [결함 패턴 1] 동일 대사 verbatim 복사 (8자 이상, 작품 전체)      │
└─────────────────────────────────────────────────────────────────┘
캐릭터 시그니처 표현을 살리라는 처방을 받으면, LLM이 같은 대사를 그대로
복사하는 경향.
...
S#38 (낙찰 직전): 수현 "(중얼) 34에서 28로. 28에서 22로..."   ← 결단
...
2. ADD 씬은 새 씬 ID(S#신규-N 또는 S#XX.5 등)를 부여하고, 기존 씬 ID 재사용 금지.
...
┌─────────────────────────────────────────────────────────────────┐
│ [결함 패턴 4] 짧은 핵심 응답 라인의 무변주 반복 ★v3.3.7 신규    │
└─────────────────────────────────────────────────────────────────┘
8자 미만의 짧은 라인이라 결함 패턴 1 검출망(8자↑)을 빠져나가지만,
관객이 가장 강하게 기억하는 "단답형 핵심 응답"이 작품 전체에 그대로
반복되면 무게감이 분산된다.

❌ 금지 (「오랜만에」 R2 실제 결함):
S#23: 준호 "처음부터요."   ← 첫 고백. 작품의 시그니처 라인.
S#34: 준호 "처음부터요."   ← 글자 그대로 반복
S#52: 준호 "처음부터요."   ← 한 번 더. 의미가 바닥남

✅ 변주 — 시그니처 보존 + 어미·단어 교체:
S#23 (첫 고백):       준호 "처음부터요."
S#34 (재확인 추궁):   준호 "처음부터, 다."
S#52 (결말 회귀):     준호 "그 처음이요."

원칙:
1. 짧은 응답 라인(2~7자)도 작품 전체에서 정규화 비교했을 때 동일 화자가
   3회 이상 같은 라인을 말하면 결함이다.
2. 시그니처 라인은 "변주 가능한 골격"으로 설계하고, 매 등장마다 어미·
   조사·강세 단어를 1개 이상 교체하라.
3. 의도적 모티프 회귀는 정확히 2회까지만 허용 (수미상관). 3회 이상은
   변주가 강제된다.

자동 후처리: auto_fix_a33_violations Phase 4에서 2~7자 라인 동일 화자 3회+
검출 → 경고 출력. (자동 변주 불가, 작가 결정 영역)

┌─────────────────────────────────────────────────────────────────┐
│ [결함 패턴 5] 씬 간 미러링 다이얼로그 패턴 복사 ★v3.3.7 신규    │
└─────────────────────────────────────────────────────────────────┘
두 씬이 의도적으로 미러링 구조여야 할 때 — LLM이 "구조 미러링"을
"라인 복사"로 오해해 5쌍 이상의 다이얼로그 시퀀스를 거의 그대로
복제하는 결함. 단일 라인은 8자↑ 검출망을 통과하지 못하지만, 시퀀스
전체로 보면 명백한 verbatim이다.

❌ 금지 (「오랜만에」 R2 실제 결함, S#33↔S#42):
S#33                                S#42
─────────────                      ─────────────
수현: 옆 건물이네요.                수현: 다 나간다는 말이요?
준호: 알고 있었어요?                준호: 여긴 안 나가요.
수현: 그러면 왜...                  수현: 그럼 진짜 안 나가요?
준호: 독서 모임 손님이라고...       준호: 네.
(5쌍 동일 패턴 + 같은 액션 비트 "행주로 카운터를 닦는다")

✅ 룰 — 미러링 씬은 "구조"만 미러, "내용"은 진화:
1. 두 씬을 미러로 짤 때, 다이얼로그 라인 자체는 절대 복사하지 마라.
2. 미러링은 "공간/소품/액션 비트"의 구조에서만 보이게 하라.
   - S#33: 카운터 위 봉투를 사이에 둠 → S#42: 카운터 위 찻잔을 사이에 둠
   - S#33: 준호가 닦는다 (회피의 액션) → S#42: 준호가 따른다 (호의의 액션)
3. 두 씬 사이에 "같은 화자의 동일 라인"이 2개 이상 있으면 즉시 결함.
4. 미러링은 **반응의 정반대**여야 한다. 같은 반응 반복은 미러링이 아닌
   plagiarism이다.

자동 후처리: auto_fix_a33_violations Phase 5에서 두 씬 사이 동일 화자
동일 라인 2개+ 검출 → 결함 보고서 출력.

┌─────────────────────────────────────────────────────────────────┐
│ [결함 패턴 6] 모티프 객체·소품 반복 (영화 동력 모순) ★v3.3.7   │
└─────────────────────────────────────────────────────────────────┘
대사가 아닌 액션 비트의 핵심 소품(motif object)이 같은 의미로 3씬
이상 반복되면 영화의 추진력이 정지한다. R2의 "이사 박스" 결함이 그 예.

❌ 금지 (「오랜만에」 R2 실제 결함):
S#14: 준호가 박스에서 책을 꺼낸다.    ← 1회: "정착의 메타포"
S#23: 준호가 박스를 들고 나온다.      ← 2회: 같은 메타포 반복
S#37: 준호가 박스 위에 앉아 있다.     ← 3회: 영화의 시간이 정지

같은 소품이 같은 의미층에서 반복되면 관객은 "이야기가 진행되지 않는다"고
체감한다.

✅ 룰 — 모티프 객체의 3단계 변환:
1. 모티프 소품은 작품에서 정확히 3회 이하 등장한다 (수미상관 포함).
2. 등장할 때마다 객체의 "상태"가 바뀌어야 한다.
   - 1회: 박스가 닫혀 있다 (미정착)
   - 2회: 박스가 열려 있다 (정착 중)
   - 3회: 박스가 사라졌다 (정착 완료)
3. 같은 객체를 같은 상태로 3씬 이상 등장시키면 결함이다.
4. 핵심 모티프 후보 (자동 추적 대상):
   "박스/이삿짐/봉투/서류/찻잔/행주/고양이/공고문/계약서/카운터"

자동 후처리: auto_fix_a33_violations Phase 6에서 모티프 객체 사전과
지문 매칭 → 동일 객체 3씬+ 등장 시 경고. 상태 변화는 작가 검토 영역.

┌─────────────────────────────────────────────────────────────────┐
│ [결함 패턴 7] 핵심 답변 라인의 글자 단위 복제 ★v3.3.7 신규     │
└─────────────────────────────────────────────────────────────────┘
질문에 대한 답으로 등장한 라인이 — 단답이든 장문이든 — 글자 한 자
틀리지 않고 다른 씬에 다시 등장하는 결함. 결함 패턴 1·4와 다른 점은,
이 패턴은 답변 라인이라는 함수적 위치에 한정된다. 답변 위치는 관객
기억에 가장 깊이 새겨지는 자리라 verbatim 반복이 가장 치명적.

❌ 금지 (「오랜만에」 R2 실제 결함):
S#33: 수현 "...알고 있었어요?"
      준호 "독서 모임 손님이라고 하셨잖아요."   ← 첫 답변 위치
S#42: 수현 "그러면 왜 들어오게 했어요?"
      준호 "독서 모임 손님이라고 하셨잖아요."   ← 글자 그대로 복제

✅ 룰 — 답변 라인은 항상 일회성:
1. 직전 라인이 의문문(?로 끝남)일 때, 그 다음 라인은 "답변 위치"이다.
2. 같은 화자의 동일 답변 라인이 작품 전체에 2회 이상 등장하면 결함이다.
   (5자 이상의 모든 답변 라인에 적용 — 결함 패턴 1보다 임계값 낮춤)
3. 의도적 회귀가 필요하면 답변 라인의 어미·강세 단어를 바꿔라.
   - 1회: "독서 모임 손님이라고 하셨잖아요."
   - 2회: "그때 손님이라고 하셨고요."

자동 후처리: auto_fix_a33_violations Phase 7에서 의문문→평서문 시퀀스를
탐지해 답변 라인 위치를 추출하고, 5자+ 동일 답변 라인 2회+ 검출 시
결함 보고서 출력.

═══════════════════════════════════════════════════════════════════
[A33 자가 점검 — 출력 직전 확인 (v3.3.7 강화 7항목)]

매 씬 작성 후, 다음 7가지를 체크하라:

1. 같은 씬 ID가 두 번 등장하지 않는가?                    [패턴 2]
2. 같은 씬 안에서 동일 화자 동일 대사 중복이 없는가?       [패턴 3]
3. 작품 전체에서 8자 이상 동일 대사가 verbatim 반복되지    [패턴 1]
   않는가?
4. 짧은 시그니처 라인(2~7자)이 동일 화자에게서 3회 이상     [패턴 4]
   반복되지 않는가?
5. 두 씬이 미러링 구조일 때, 동일 라인이 2개 이상 공유      [패턴 5]
   되지 않는가? (구조만 미러, 라인은 진화)
6. 핵심 모티프 객체(박스·봉투·찻잔 등)가 3씬 이상 같은       [패턴 6]
   상태로 등장하지 않는가?
7. 의문문 직후의 답변 라인이 작품 전체에서 verbatim 복제    [패턴 7]
   되지 않는가?

7항목 중 하나라도 실패하면 즉시 변주 또는 삭제.

═══════════════════════════════════════════════════════════════════
[A34: CONCRETE EVASION — 우회는 구체로, 메타포 금지]
v3.3.6 신규 — 「오랜만에」 S#35에서 발견된 LLM 결함 방어.

[결함 패턴]
인물이 직접적인 질문을 받았을 때, LLM이 시적이고 추상적인 메타포로 답하는 경향.
시처럼 들리지만 실제 의미가 모호하고, 질문에 답이 안 됨.

❌ 금지 — 추상 메타포 우회 (의미 모호):
수현    그래서 안 팔았던 거예요? 저 때문에?
준호    빈 자리가 있으면 채우는 게 서점이에요.   ← 무슨 뜻?
수현    서점이요?
준호    빈 의자는 채워야 하고, 빈 잔에는 차를 따라야 하고.   ← 동문서답

이런 대사는 시처럼 들리지만:
- 수현의 구체적 질문에 답이 안 됨
- "빈 자리/빈 의자/빈 잔" 메타포가 작품 세계와 연결되지 않음
- 독자/관객이 의미를 추측해야 함 → 몰입 깨짐

✅ 우회 4유형 (이 중 하나만 사용):

[유형 1] 행동 우회 — 대사 없이 행동만
수현    그래서 안 팔았던 거예요? 저 때문에?
준호가 찻잔을 천천히 닦는다. 답하지 않는다.
수현    (잠시 기다린다.) 아니죠?

[유형 2] 사실 우회 — 구체적 숫자·날짜·사물명
수현    그래서 안 팔았던 거예요? 저 때문에?
준호    임대 계약이 8개월 남았어요.    ← 구체적 사실로 회피
수현    그게 답이에요?
준호    네.
...
[유형 4] 직업/역할 우회 — "내 일이라서" 형태
수현    그래서 왜 자리를 줬어요?
준호    그게 제 일이니까요. 손님 안내하고, 차 내고, 책 권하는 거.

[자가 점검 — 매 대사마다]
시 같은 대사를 쓰고 싶을 때 자문하라:
1. 이 메타포가 작품 세계 안에서 명확한 의미를 가지는가?
2. 질문에 직접적이지 않더라도 답의 방향성이 들리는가?
3. 이 메타포 없이 똑같은 의미를 전달할 수 있는가?
   → 가능하면 메타포 제거.
4. "빈 X은 채워야 하고", "Y는 어디서나 분다" 같은
   일반 명제 + 시적 톤 조합이 보이면 즉시 재작성.

[예외 — 시적 표현 허용 케이스]
- 캐릭터의 직업이 시인·작가일 때 (직업적 일관성)
- 회상/플래시백 안에서 (시간적 거리감 표현)
- 결말부 한 줄 (작품의 주제 응축)
- 이 외에는 일반 대사에서 시적 메타포 자제.

═══════════════════════════════════════════════════════════════════
[A35: 씬 헤딩 시간대 표기 — 5단계 한글 표기 강제]
v3.4.0 신규 — 「상속」 R2에서 발견된 시간대 표기 혼용 결함 방어.

[결함 패턴]
씬 헤딩의 시간대가 'DAY', 'NIGHT' 영문과 '낮', '밤', '점심', '아침',
'해질녘' 등 다양한 한글이 뒤섞여 표기됨. 표기가 들쭉날쭉하면 프로덕션
스케줄(조명·촬영 시간대 분류)에서 혼란이 생긴다.

[규칙 — 시간대는 반드시 아래 5개 중 하나만 사용한다]

  새벽 / 오전 / 오후 / 저녁 / 밤

이 외의 어떤 시간대 표기도 출력 금지. 씬 헤딩 형식:
...
[매핑 기준]
- DAY / 낮 / 주간 → 씬 맥락으로 오전·오후 판단.
  (앞 씬의 시간 흐름, 식사·업무·해의 위치 단서로 결정.
   단서가 전혀 없으면 '오후'를 기본값으로 한다.)
- 아침 / 이른 아침 / 동틀 무렵 → 오전
- 점심 / 정오 / 한낮 → 오후
- 해질녘 / 황혼 / 노을 / 땅거미 / 일몰 → 저녁
- NIGHT / 밤 / 야간 / 한밤중 / 자정 → 밤
...
[자가 점검 — 씬 헤딩을 쓸 때마다]
1. 시간대가 5개(새벽/오전/오후/저녁/밤) 중 하나인가?
2. 영문(DAY/NIGHT)이나 다른 한글(낮/아침/점심 등)이 섞이지 않았는가?
3. 장소와 시간대 사이 구분자가 ' — '(공백+엠대시+공백)인가?
...
{{
  "verify_report": {{
    ...
    "ai_escape_check": {{
      ...
      "clean_patterns": "위반이 없는 패턴 개수 (총 34개 중)"
    }},
